import {
  ...
  instructorCalendars,
  blockedTimeCache,
  type InstructorGoogleCredentials,
  type InsertInstructorGoogleCredentials,
  type InstructorCalendar,
  type InsertInstructorCalendar,
  type BlockedTimeCache,
  type InsertBlockedTimeCache,
...
export interface IStorage {
  ...
  // ============================================
  // PER-INSTRUCTOR GOOGLE CALENDAR METHODS
  // ============================================

  // Instructor Google credentials operations
  getInstructorGoogleCredentials(instructorId: string): Promise<InstructorGoogleCredentials | undefined>;
  saveInstructorGoogleCredentials(credentials: InsertInstructorGoogleCredentials): Promise<InstructorGoogleCredentials>;
  updateInstructorGoogleCredentials(instructorId: string, data: Partial<InsertInstructorGoogleCredentials>): Promise<InstructorGoogleCredentials>;
  deleteInstructorGoogleCredentials(instructorId: string): Promise<void>;

  // Instructor calendar selections operations
  getInstructorCalendars(instructorId: string): Promise<InstructorCalendar[]>;
  getBlockingCalendars(instructorId: string): Promise<InstructorCalendar[]>;
  saveInstructorCalendar(calendar: InsertInstructorCalendar): Promise<InstructorCalendar>;
  updateInstructorCalendar(id: string, data: Partial<InsertInstructorCalendar>): Promise<InstructorCalendar>;
  deleteInstructorCalendar(id: string): Promise<void>;
  deleteAllInstructorCalendars(instructorId: string): Promise<void>;

  // Blocked time cache operations
  getBlockedTimeCache(instructorId: string, startTime: Date, endTime: Date): Promise<BlockedTimeCache[]>;
  saveBlockedTimeCache(entries: InsertBlockedTimeCache[]): Promise<void>;
  clearBlockedTimeCache(instructorId: string): Promise<void>;
  clearExpiredBlockedTimeCache(): Promise<void>;

  // Google Calendar toggle operations
  updateGoogleCalendarSettings(instructorId: string, settings: {
    googleCalendarConnected?: boolean;
    googleCalendarSyncEnabled?: boolean;
    googleCalendarBlockingEnabled?: boolean;
    timezone?: string;
  }): Promise<User>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // ============================================
  // PER-INSTRUCTOR GOOGLE CALENDAR IMPLEMENTATIONS
  // ============================================

  // Instructor Google credentials operations
  async getInstructorGoogleCredentials(instructorId: string): Promise<InstructorGoogleCredentials | undefined> {
    const [creds] = await db
      .select()
      .from(instructorGoogleCredentials)
      .where(eq(instructorGoogleCredentials.instructorId, instructorId));
    return creds;
  }

  async saveInstructorGoogleCredentials(credentials: InsertInstructorGoogleCredentials): Promise<InstructorGoogleCredentials> {
    const existing = await this.getInstructorGoogleCredentials(credentials.instructorId);
    
    if (existing) {
      const [updated] = await db
        .update(instructorGoogleCredentials)
        .set({
          accessToken: credentials.accessToken,
          refreshToken: credentials.refreshToken,
          tokenExpiry: credentials.tokenExpiry,
          primaryCalendarId: credentials.primaryCalendarId || 'primary',
          updatedAt: new Date(),
        })
        .where(eq(instructorGoogleCredentials.instructorId, credentials.instructorId))
        .returning();
      return updated;
    }
    
    const [created] = await db
      .insert(instructorGoogleCredentials)
      .values(credentials)
      .returning();
    return created;
  }

  async updateInstructorGoogleCredentials(instructorId: string, data: Partial<InsertInstructorGoogleCredentials>): Promise<InstructorGoogleCredentials> {
    const [updated] = await db
      .update(instructorGoogleCredentials)
  ...
  async deleteInstructorGoogleCredentials(instructorId: string): Promise<void> {
    await db
      .delete(instructorGoogleCredentials)
      .where(eq(instructorGoogleCredentials.instructorId, instructorId));
  }

  // Instructor calendar selections operations
  async getInstructorCalendars(instructorId: string): Promise<InstructorCalendar[]> {
    return db
      .select()
      .from(instructorCalendars)
      .where(eq(instructorCalendars.instructorId, instructorId))
      .orderBy(desc(instructorCalendars.isPrimary));
  }

  async getBlockingCalendars(instructorId: string): Promise<InstructorCalendar[]> {
    return db
      .select()
      .from(instructorCalendars)
      .where(and(
        eq(instructorCalendars.instructorId, instructorId),
        eq(instructorCalendars.blocksAvailability, true)
      ));
  }

  async saveInstructorCalendar(calendar: InsertInstructorCalendar): Promise<InstructorCalendar> {
    const [created] = await db
      .insert(instructorCalendars)
      .values(calendar)
      .onConflictDoUpdate({
        target: [instructorCalendars.instructorId, instructorCalendars.calendarId],
        set: {
          calendarName: calendar.calendarName,
          calendarColor: calendar.calendarColor,
          blocksAvailability: calendar.blocksAvailability,
  ...
  async updateInstructorCalendar(id: string, data: Partial<InsertInstructorCalendar>): Promise<InstructorCalendar> {
    const [updated] = await db
      .update(instructorCalendars)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(instructorCalendars.id, id))
      .returning();
    return updated;
  }

  async deleteInstructorCalendar(id: string): Promise<void> {
    await db.delete(instructorCalendars).where(eq(instructorCalendars.id, id));
  }

  async deleteAllInstructorCalendars(instructorId: string): Promise<void> {
    await db.delete(instructorCalendars).where(eq(instructorCalendars.instructorId, instructorId));
  }

  // Blocked time cache operations
  async getBlockedTimeCache(instructorId: string, startTime: Date, endTime: Date): Promise<BlockedTimeCache[]> {
    return db
      .select()
      .from(blockedTimeCache)
      .where(and(
        eq(blockedTimeCache.instructorId, instructorId),
        gte(blockedTimeCache.startTimeUtc, startTime),
        sql`${blockedTimeCache.endTimeUtc} <= ${endTime}`,
        gte(blockedTimeCache.expiresAt, new Date())
      ))
      .orderBy(asc(blockedTimeCache.startTimeUtc));
  }

  async saveBlockedTimeCache(entries: InsertBlockedTimeCache[]): Promise<void> {
    if (entries.length === 0) return;
    await db.insert(blockedTimeCache).values(entries);
  }

  async clearBlockedTimeCache(instructorId: string): Promise<void> {
    await db.delete(blockedTimeCache).where(eq(blockedTimeCache.instructorId, instructorId));
  }

  async clearExpiredBlockedTimeCache(): Promise<void> {
    await db.delete(blockedTimeCache).where(sql`${blockedTimeCache.expiresAt} < now()`);
  }

  // Google Calendar toggle operations
  async updateGoogleCalendarSettings(instructorId: string, settings: {
    googleCalendarConnected?: boolean;
    googleCalendarSyncEnabled?: boolean;
    googleCalendarBlockingEnabled?: boolean;
    timezone?: string;
