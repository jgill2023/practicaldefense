...
files_modified:
  ...
  - client/src/components/CourseCreationForm.tsx
  - client/src/components/EditCourseForm.tsx
  - client/src/components/EventCreationForm.tsx
...
export async function getObjectAclPolicy(
  ...
  return { owner: rows[0].ownerId, visibility: rows[0].visibility as "public" | "private", blobUrl: rows[0].blobUrl };
}
...
<task type="auto">
  ...
  <files>server/routes.ts, server/giftCards/routes.ts, server/index.ts, client/src/components/ObjectUploader.tsx, client/src/components/CommunicationsDashboard.tsx, client/src/components/CourseCreationForm.tsx, client/src/components/EditCourseForm.tsx, client/src/components/EventCreationForm.tsx, client/src/pages/product-management.tsx, .env.example</files>
  <action>
...
**b. Lines ~3444-3470 — Object download endpoint:**

The object download endpoint (`GET /objects/*`) currently creates `ObjectStorageService`, calls `getObjectEntityFile()` and `canAccessObjectEntity()` and `downloadObject()`. Rewrite to use the `blobUrl` column added to the `blobAclPolicies` table in Task 1.

The approach: the download route accepts a blob pathname, looks up the ACL record (which contains the full Vercel Blob URL in the `blobUrl` column), checks access, then streams the blob through Express.

Rewrite the download route:
```typescript
...
**3. Update client/src/components/ObjectUploader.tsx and ALL callers:**

Remove the `onGetUploadParameters` prop entirely. ObjectUploader now handles upload internally via FormData POST to `/api/objects/upload`. The component no longer needs callers to provide upload URL logic.

Updated ObjectUploader behavior:
- Remove `onGetUploadParameters` from `ObjectUploaderProps` interface
- Inline the upload logic: build `FormData`, POST to `/api/objects/upload`, parse response for `{ uploadURL, pathname }`
- Keep `onUploadComplete` callback so callers receive the resulting URL/pathname
- Keep `acceptedTypes`, `children`, and other existing props

```typescript
// Inside ObjectUploader's upload handler:
...
// Call onUploadComplete with data.uploadURL / data.pathname
```

Update ALL callers to remove `onGetUploadParameters` callbacks:
- `client/src/components/CourseCreationForm.tsx` — remove `handleGetUploadParameters` function and `onGetUploadParameters` prop from `<ObjectUploader>`
- `client/src/components/EditCourseForm.tsx` — remove `handleGetUploadParameters` function and `onGetUploadParameters` prop from `<ObjectUploader>`
- `client/src/components/CommunicationsDashboard.tsx` — remove `onGetUploadParameters` inline callbacks from both `<ObjectUploader>` instances
- `client/src/pages/product-management.tsx` — remove `onGetUploadParameters` inline callback from `<ObjectUploader>`
- `client/src/components/EventCreationForm.tsx` — remove unused `ObjectUploader` import (dead import, component is not rendered)
