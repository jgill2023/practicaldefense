...
Output: server/app.ts (Express factory), api/index.ts (Vercel entry), refactored server/index.ts (local dev only).
</objective>
...
<task type="auto">
  <name>Task 2: Create api/index.ts Vercel serverless entry point</name>
  <files>api/index.ts</files>
  <action>
Create `api/index.ts` at the project root (NOT in server/):
   ```typescript
