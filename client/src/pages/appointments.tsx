...
export default function AppointmentsPage() {
  ...
  const { data: availability, isLoading: availabilityLoading, refetch: refetchAvailability } = useQuery<{
    ...
    queryKey: [`/api/availability/${dateString}?instructorId=${instructorId}&slotDuration=${slotDuration}`],
    enabled: !!instructorId && isAuthenticated,
  });

  const blocksEndDate = format(addDays(selectedDate, 30), "yyyy-MM-dd");
  const { data: blocksData, isLoading: blocksLoading } = useQuery<{ blocks: ManualBlock[] }>({
    queryKey: [`/api/availability/instructor/blocks?startDate=${dateString}&endDate=${blocksEndDate}`],
    enabled: !!instructorId && isAuthenticated,
  });
  
  const blocks = blocksData || { blocks: [] };
  const selectedDateBlocks = blocks.blocks.filter(block => {
  ...
  const createBlockMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setBlockReason("");
      queryClient.invalidateQueries({ queryKey: [`/api/availability/${dateString}?instructorId=${instructorId}&slotDuration=${slotDuration}`] });
      queryClient.invalidateQueries({ queryKey: [`/api/availability/instructor/blocks?startDate=${dateString}&endDate=${blocksEndDate}`] });
    },
    onError: (error: any) => {
      toast({
        title: "Error",
      ...
      <div className="container mx-auto px-12 md:px-16 lg:px-24 py-8">
        ...
        <Tabs defaultValue="types" className="space-y-6">
          ...
          <TabsContent value="availability">
            ...
            {/* Manual Blocks Section - Selected Date */}
            <Card className="mt-6">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span className="flex items-center gap-2">
                  ...
                  <Button size="sm" onClick={() => setShowBlockDialog(true)} data-testid="button-add-block">
                    <Plus className="h-4 w-4 mr-1" />
                    Add Block
                  </Button>
                </CardTitle>
                <CardDescription>Time-off blocks for the selected date</CardDescription>
              </CardHeader>
              <CardContent>
                {blocksLoading ? (
                  ...
                  <div className="space-y-2">
                    {selectedDateBlocks.map((block, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-3 rounded-md bg-red-100 dark:bg-red-900/40 border border-red-300 dark:border-red-700"
                      ...
                      >
                        ...
                        <Badge variant="secondary" className="bg-red-200 dark:bg-red-700">
                          Blocked
                        </Badge>
                      </div>
                    ))}
                  ...
                  <div className="text-center py-4 text-muted-foreground" data-testid="text-no-blocks">
                    No blocks for this date. Use "Add Block" to mark time off.
                  </div>
                )}
              </CardContent>
            </Card>

            {/* Other Upcoming Blocks */}
            {blocks.blocks.length > selectedDateBlocks.length && (
              <Card className="mt-4">
                ...
                <CardContent className="pt-0">
                  <div className="space-y-2">
                    {blocks.blocks
                      .filter(block => format(parseISO(block.startTime), "yyyy-MM-dd") !== dateString)
                      .slice(0, 5)
                      .map((block, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between p-2 rounded-md bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm"
                        >
                          <div className="flex items-center gap-2">
                            <CalendarX2 className="h-3 w-3 text-red-500" />
                            <span>
                              {format(parseISO(block.startTime), "MMM d")} - {formatTime(block.startTime)} to {formatTime(block.endTime)}
                            </span>
                          </div>
                        </div>
                      ))}
