...
import { Users, Phone, Mail, Calendar, DollarSign, X, FileText, AlertCircle, Award, RotateCcw, MessageSquare, FileSignature, UserPlus, ArrowLeft } from "lucide-react";
...
import { ViewCompletedWaiverModal } from "@/components/ViewCompletedWaiverModal";
import { ViewCompletedFormsModal } from "@/components/ViewCompletedFormsModal";
...
export default function RosterPage() {
  ...
  const { data: rosterData, isLoading, error } = useQuery<RosterData>({
    queryKey: ["/api/instructor/roster", scheduleId, null],
    queryFn: async () => {
  ...
  const addStudentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/instructor/roster", scheduleId, null] });
  ...
  const handlePaymentClick = (_studentId: string, _studentName: string, enrollmentId: string) => {
    setLocation('/payment-details/' + enrollmentId);
  };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">No schedule ID was provided.</p>
                              ...
                              if (isComplete) {
                                ...
                                return (
                                  <Button
                                    ...
                                    size="sm"
                                    className="h-8 w-8 p-0 hover:bg-green-100 dark:hover:bg-green-900/20"
                                    onClick={() => setViewFormsModal({ isOpen: true, enrollmentId: student.enrollmentId, courseId: '', studentName: `${student.firstName} ${student.lastName}` })}
